...
- **Home Assistant's frontend**, in each user's profile language: dialogs and forms, attribute names, group subtitles, repair issues (`issues`, the upgrade details), the reason setup is being retried (`exceptions`, from the error's key and placeholders), and the error of a `bitpanda.refresh` call made in the UI while no entry is loaded (`exceptions.nothing_to_refresh`, raised with its key alone; the log and automation traces show its English text, and the entry's language option does not apply — no entry is loaded). Never write such a text out in the backend; hand the frontend its key and placeholders, and keep the placeholders free of words — entity IDs, codes, asset labels and Markdown only.
