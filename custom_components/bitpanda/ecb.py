...
from .const import API_TIMEOUT, ECB_RATES_URL
...
def parse_ecb_rates(document: str | bytes) -> EcbRates:
    ...
    try:
        ...
        raise EcbError("The ECB response is not valid XML") from None
    day = root.find(".//eurofxref:Cube[@time]", _NS)
    if day is None:
        raise EcbError("The ECB response carries no rate date")
    rates: dict[str, float] = {}
    ...
    if not rates:
        raise EcbError("The ECB response carries no rates")
...
async def async_fetch_ecb_rates(session: aiohttp.ClientSession) -> EcbRates:
    ...
    try:
        ...
        raise EcbError(f"HTTP {err.status} from the ECB") from None
    except aiohttp.ClientError as err:
        raise EcbError(f"Connection error for the ECB rates: {type(err).__name__}") from None
    except asyncio.TimeoutError:
        raise EcbError("Timeout fetching the ECB rates") from None
